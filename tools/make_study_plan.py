...
out += [
 "## Недельные проекты — сделать руками на своей карте",
 "",
 "Дневные задания выше — карандаш и бумага. Это — код: по одному проекту на неделю,",
 "вечера и выходные. Именно их спрашивают на coding screen в сильных командах, и именно",
 "они превращают «читал про X» в «делал X». Порядок совпадает с неделями плана.",
 "",
 "1. **Неделя 1 — трансформер с нуля (nanoGPT).** Свой BPE-токенизатор (обучить на",
 "   любимом корпусе, посмотреть fertility), attention/MHA/RoPE руками на PyTorch без",
 "   `nn.Transformer`, обучить мини-GPT на своей карте до связного текста. Критерий:",
 "   можешь объяснить каждую строчку и вывести форму каждого тензора по памяти.",
 "2. **Неделя 2 — RAG-стек и эмбеддинги.** Мини-RAG над своими документами: чанкинг →",
 "   bge/e5 → FAISS (Flat против HNSW с замером recall/latency) → реранкер → генерация",
 "   с цитатами. Собрать eval на 50 вопросов и померить recall@k до/после реранкера.",
 "3. **Неделя 3 — тренировка и пост-тренинг.** LoRA-дообучение 7–8B на своей карте:",
 "   замерить память при r=16/64, с checkpointing и без — сверить с расчётом 16 Б/параметр;",
 "   затем DPO на паре сотен пар с абляцией beta. Логи и графики сохранить — это материал",
 "   для интервью.",
 "4. **Неделя 4 — inference и evals.** Поднять vLLM с той же моделью: замерить токены/с",
 "   при батче 1/8/32, посчитать KV-cache и сверить с формулой, попробовать int8/int4 и",
 "   прогнать свой eval до/после квантизации. Плюс мини-red-teaming: 20 атакующих",
 "   промптов на свой RAG из недели 2, находки — в регресс-набор.",
 "",
]
